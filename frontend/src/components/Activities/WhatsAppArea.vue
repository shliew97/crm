<template>
  <div>
    <div
      ...
      :key="whatsapp.name">
      <template v-if="isNewDate(index)">
        <div class="date-line text-gray-600 text-sm font-bold">{{ getDisplayDate(whatsapp.timestamp) }}</div>
      </template>
      <div
      ...
      >
        <div
          :id="whatsapp.name"
          class="group/message relative max-w-[50%] prose-f rounded-md bg-gray-50 p-1.5 pl-2 text-base shadow-sm"
        >
          <div
...
function isNewDate(index) {
  ...
  const current = new Date(props.messages[index].timestamp).toDateString();
  const prev = new Date(props.messages[index - 1].timestamp).toDateString();
  return current !== prev;
}
