<template>
  ...
  <teleport to="body">
    ...
    >
      <div class="w-full max-w-md rounded-2xl bg-gray-50 p-5 shadow-xl">
        <!-- Header -->
        ...
        <div v-if="activeSuggestedSlots.slot_1.length == 0 && activeSuggestedSlots.slot_2.length == 0 && activeSuggestedSlots.slot_3.length == 0 && activeSuggestedSlots.slot_4.length == 0 && activeSuggestedSlots.slot_5.length == 0 && activeSuggestedSlots.slot_6.length == 0" class="flex justify-center py-8">
          <span class="text-2xl font-semibold text-gray-500">{{ __('No suggestion available') }}</span>
        </div>

        <div class="mt-4 flex flex-col gap-4">
          <!-- Same Outlet Card -->
          <div v-if="activeSuggestedSlots.slot_1.length > 0" class="overflow-hidden rounded-xl">
            <div class="flex items-center gap-1.5 bg-green-100 px-4 py-2.5">
              <span class="text-xs font-semibold uppercase tracking-wide text-green-700">📅{{ formatSlotDate(activeSuggestedSlots.slot_1[0].booking_date) }}</span>
              <span class="text-xs font-bold uppercase tracking-wide text-green-700">{{ activeSuggestedSlots.slot_1[0].package_available ? '✅' : '❌' }}&nbsp;{{ activeSuggestedSlots.slot_1[0].shop_full_name }}</span>
            </div>
            <div class="bg-white p-4">
              <div class="flex items-start gap-4">
                <div class="text-3xl font-bold bg-green-100 text-green-700 rounded-xl p-2.5 cursor-pointer" @click="setWhatsAppMessage(activeSuggestedSlots.slot_1_message)">
                ...
                <div v-for="(slot, index) in activeSuggestedSlots.slot_1" class="text-3xl font-bold bg-green-100 text-green-700 rounded-xl p-2.5 cursor-pointer" @click="selectSuggestedSlot(slot)">
          ...
          <!-- Nearby Outlet Card -->
          <div v-for="(slot, index) in activeSuggestedSlots.slot_2" class="overflow-hidden rounded-xl">
          ...
          <div v-if="activeSuggestedSlots.slot_3.length > 0" class="overflow-hidden rounded-xl">
            <div class="flex items-center gap-1.5 bg-green-100 px-4 py-2.5">
              <span class="text-xs font-semibold uppercase tracking-wide text-green-700">
                ...
                {{ formatSlotDate(activeSuggestedSlots.slot_3[0].booking_date) }}
              </span>
              <span class="text-xs font-bold uppercase tracking-wide text-green-700">{{ activeSuggestedSlots.slot_3[0].package_available ? '✅' : '❌' }}&nbsp;{{ activeSuggestedSlots.slot_3[0].shop_full_name }}</span>
            </div>
            <div class="bg-white p-4">
              <div class="flex items-start gap-4">
                <div class="text-3xl font-bold bg-green-100 text-green-700 rounded-xl p-2.5 cursor-pointer" @click="setWhatsAppMessage(activeSuggestedSlots.slot_3_message)">
                  <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M7 9H17M7 13H17M21 20L17.6757 18.3378C17.4237 18.2118 17.2977 18.1488 17.1656 18.1044C17.0484 18.065 16.9277 18.0365 16.8052 18.0193C16.6672 18 16.5263 18 16.2446 18H6.2C5.07989 18 4.51984 18 4.09202 17.782C3.71569 17.5903 3.40973 17.2843 3.21799 16.908C3 16.4802 3 15.9201 3 14.8V7.2C3 6.07989 3 5.51984 3.21799 5.09202C3.40973 4.71569 3.71569 4.40973 4.09202 4.21799C4.51984 4 5.0799 4 6.2 4H17.8C18.9201 4 19.4802 4 19.908 4.21799C20.2843 4.40973 20.5903 4.71569 20.782 5.09202C21 5.51984 21 6.0799 21 7.2V20Z" stroke="rgb(19 121 73" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                </div>
                <div v-for="(slot, index) in activeSuggestedSlots.slot_3" class="text-3xl font-bold bg-green-100 text-green-700 rounded-xl p-2.5 cursor-pointer" @click="selectSuggestedSlot(slot)">
                  {{ formatSlotTime(slot.timeslot) }}
                </div>
              </div>
            </div>
          ...
          <div v-for="(slot, index) in activeSuggestedSlots.slot_4" class="overflow-hidden rounded-xl">
            <div class="flex items-center gap-1.5 bg-violet-100 px-4 py-2.5">
          ...
          <div v-if="activeSuggestedSlots.slot_5.length > 0" class="overflow-hidden rounded-xl">
            <div class="flex items-center gap-1.5 bg-green-100 px-4 py-2.5">
              <span class="text-xs font-semibold uppercase tracking-wide text-green-700">
                <span class="text-xs font-semibold uppercase tracking-wide text-green-100 bg-green-700 px-1">NW</span>
                {{ formatSlotDate(activeSuggestedSlots.slot_5[0].booking_date) }}
              </span>
              <span class="text-xs font-bold uppercase tracking-wide text-green-700">{{ activeSuggestedSlots.slot_5[0].package_available ? '✅' : '❌' }}&nbsp;{{ activeSuggestedSlots.slot_5[0].shop_full_name }}</span>
            </div>
            <div class="bg-white p-4">
              <div class="flex items-start gap-4">
                <div class="text-3xl font-bold bg-green-100 text-green-700 rounded-xl p-2.5 cursor-pointer" @click="setWhatsAppMessage(activeSuggestedSlots.slot_5_message)">
                  <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M7 9H17M7 13H17M21 20L17.6757 18.3378C17.4237 18.2118 17.2977 18.1488 17.1656 18.1044C17.0484 18.065 16.9277 18.0365 16.8052 18.0193C16.6672 18 16.5263 18 16.2446 18H6.2C5.07989 18 4.51984 18 4.09202 17.782C3.71569 17.5903 3.40973 17.2843 3.21799 16.908C3 16.4802 3 15.9201 3 14.8V7.2C3 6.07989 3 5.51984 3.21799 5.09202C3.40973 4.71569 3.71569 4.40973 4.09202 4.21799C4.51984 4 5.0799 4 6.2 4H17.8C18.9201 4 19.4802 4 19.908 4.21799C20.2843 4.40973 20.5903 4.71569 20.782 5.09202C21 5.51984 21 6.0799 21 7.2V20Z" stroke="rgb(19 121 73" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                </div>
                <div v-for="(slot, index) in activeSuggestedSlots.slot_5" class="text-3xl font-bold bg-green-100 text-green-700 rounded-xl p-2.5 cursor-pointer" @click="selectSuggestedSlot(slot)">
                  {{ formatSlotTime(slot.timeslot) }}
                </div>
              </div>
            </div>
          ...
          <div v-for="(slot, index) in activeSuggestedSlots.slot_6" class="overflow-hidden rounded-xl">
            <div class="flex items-center gap-1.5 bg-violet-100 px-4 py-2.5">
              <span class="text-xs font-semibold uppercase tracking-wide text-violet-700">
...
function searchMemberAccount() {
  ...
  if (!bookingForm.value.booking_date) {
    createToast({
      title: __('Missing Info'),
      text: __('Please select booking date first'),
      icon: 'alert-triangle',
      iconClasses: 'text-yellow-600',
    })
    return
...
function setWhatsAppMessage(message) {
  activities.value.content = message;
  showSuggestedSlots.value = false;
}
</script>
